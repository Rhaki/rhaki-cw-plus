...
pub struct RouterWrapper<'a> {
    closure: &'a dyn Fn(UseRouter) -> AnyResult<UseRouterResponse>,
}

impl<'a> RouterWrapper<'a> {
    pub fn new(closure: &'a dyn Fn(UseRouter) -> AnyResult<UseRouterResponse>) -> Self {
        Self { closure }
    }

    pub fn query<T: Serialize, R: DeserializeOwned>(&self, query: T) -> AnyResult<R> {
    ...
    pub fn execute<T: Serialize>(&self, sender: Addr, comsos_msg: T) -> AnyResult<AppResponse> {
